<?php
/**
 * Archives归档(修改自<a href="http://typecho.jkey.lu/" target="_blank">jKey</a>的0.2.2版本)
 * 
 * @package Archives
 * @author SELFTOWN
 * @version 1.0
 * @link http://selftown.com/twitter/
 * @license GPL3
 */
...
class JustArchives_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function config(Typecho_Widget_Helper_Form $form)
	{
		...
		$cfg_commentcount = new Typecho_Widget_Helper_Form_Element_Checkbox(
			'cfg_commentcount',
			array('show' => '显示评论数'),
			array('show'),
			'是否显示评论数'
	...
	<style type="text/css">
		...
		.car-toggler{font-size:12px; margin-left:0px;}
		.car-monthlisting{padding-left:20px;}
	</style>
...
<?php
	...
	public static function PostList()
	{
		...
		$atts = array(
			...
			'commentcount' => ($settings->cfg_commentcount != NULL) ? (in_array('show', $settings->cfg_commentcount) ? '1' : '0') : '0'
		);
		...
		foreach( $posts as $yearmonth => $posts ) {
			...
			foreach( $posts as $post ) {
				if ( TRUE == $firstpost ) {
					...
					if ( '0' != $atts['postcount'] ) $html .= ' <span title="Post Count">( ' . count($posts) . '篇 )</span>';
					$html .= "</span>\n		<ul class='car-monthlisting'>\n";
					$firstpost = FALSE;
				}

				$html .= '			<li>' .  $post['day'] . '日 : <a href="' . $post['permalink'] . '">' . $post['title'] . '</a>';
				...
				if ( '0' != $atts['commentcount'] && ( 0 != $post['commentsNum'] || '0' != $post['allowComment'] ) )
					$html .= ' <span title="Comment Count">( ' . $post['commentsNum'] . ' 回复 )</span>';
